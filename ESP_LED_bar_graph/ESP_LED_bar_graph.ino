...
// Pins for LED bar graph
const int LED_BAR_PINS[] = {D2, D3, D5, D6, D7, D8};
const int NUM_LEDS = 6; // Total number of LEDs in the bar graph
...
// Function to update the LED bar graph based on the light intensity level
void updateLedBarGraph(int value) {
  int ledsToLight = map(value, 0, 1023, 0, NUM_LEDS); // Map light intensity to the number of LEDs to light up
  for (int i = 0; i < NUM_LEDS; i++) {
    if (i < ledsToLight) {
      digitalWrite(LED_BAR_PINS[i], HIGH); // Turn on LED
    } else {
      digitalWrite(LED_BAR_PINS[i], LOW); // Turn off LED
    }
  }
}
...
void loop() {
  ...
  updateLedBarGraph(analogValue); // Update LED bar graph based on light intensity
